...
import { LayoutDashboard, BarChart3, Pill, User, LogOut, Shield } from "lucide-react";
import { logout } from "@/lib/auth";

export function Sidebar({ isMobile, role }: { isMobile?: boolean, role?: string | null }) {
        ...
        return (
            <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-2 py-2 flex justify-around items-center z-50 shadow-[0_-2px_10px_rgba(0,0,0,0.05)]">
                ...
                <Link href="/patients" className={`flex flex-col items-center gap-1 min-w-[64px] ${pathname === "/patients" ? "text-blue-700" : "text-gray-400"}`}>
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-tighter">Patients</span>
                </Link>
                {role === "admin" && (
                    <>
    ...
    return (
        <aside className="hidden md:flex w-64 bg-white border-r border-gray-200 flex flex-col shrink-0">
            <div className="p-6 border-b border-gray-100">
                ...
                <p className="text-xs text-gray-500 mt-1 uppercase tracking-tighter font-black opacity-50">Eye & Diabetes Clinic</p>
            </div>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                        <p className="text-sm font-medium text-gray-900 leading-tight">
                            {role === 'admin' ? 'Clinic Doctor' : 'Clinic Sister'}
                        </p>
                        <p className="text-[10px] text-gray-500 uppercase font-black tracking-widest">{role === 'admin' ? 'Doctor' : 'Sister'}</p>
                    </div>
                </div>
                <button
                    onClick={() => logout()}
                    className="w-full flex items-center gap-2 px-3 py-2 text-[10px] font-black uppercase tracking-widest text-rose-600 hover:bg-rose-50 rounded-lg transition-colors border border-transparent hover:border-rose-100"
                >
